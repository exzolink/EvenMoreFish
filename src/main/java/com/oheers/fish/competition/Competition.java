...
public class Competition {
    ...
    public void end() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            player.sendTitle(FishUtils.translateHexColorCodes("&3Соревнование завершено"), FishUtils.translateHexColorCodes("&bНовое соревнование начнется через &e3 часа"), 20, 80, 20);
    ...
    /**
     * Checks for scheduled alerts and whether the competition should end for
     * each second - this is called automatically by the competition ticker
     * every 20 ticks.
     *
    ...
    /**
     * This creates a message object and applies all the settings to it to make
     * it able to use the {type} variable. It takes into consideration whether
     * it's a specific fish/rarity competition.
     *
     * @param configMessage The configmessage to use. Must have the {type}
     * variable in it.
     * @return A message object that's pre-set to be compatible for the time
     * remaining.
     */
    ...
    /**
     * On servers with low tps, 20 ticks != 1 second, so competitions can last
     * for considerably longer, this re-calibrates the time left with an epoch
     * version of the time left. It runs through each second skipped to make
     * sure all necessary processes take place i.e. alerts.
     */
    ...
    /**
     * Calculates whether to send the "new first place" notification as an
     * actionbar message or directly into chat.
     *
    ...
    public void announceBegin() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            player.sendTitle(FishUtils.translateHexColorCodes("&3Соревнование рыбаков"), FishUtils.translateHexColorCodes("&bУ вас есть &e15 минут&b, удачной ловли"), 20, 80, 20);
        }
    }
    ...
    /**
     * The sound that gets sent to players when the competition begins, defined
     * in competitions.yml
     *
     * @param competitionName The name of the competition as stated in the
     * competitions.yml file.
     */
